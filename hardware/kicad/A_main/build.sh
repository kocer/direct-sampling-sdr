#!/usr/bin/env bash
# A karti semasini uretir, kanoniklestirir, dogrular.
#
#   ./build.sh          uret + ERC
...
echo "== ERC =="
kicad-cli sch erc dogrudan_sdr_A.kicad_sch -o /tmp/erc.rpt 2>&1 | grep -E "Found|violation" || true
grep -oP '\[.*?\]:' /tmp/erc.rpt 2>/dev/null | sort | uniq -c | sort -rn | sed 's/^/   /' || true

if [ "$1" = "pdf" ]; then
    echo "== cikti =="
    kicad-cli sch export pdf dogrudan_sdr_A.kicad_sch -o /tmp/A.pdf >/dev/null
    rm -rf /tmp/A_svg && mkdir -p /tmp/A_svg
    kicad-cli sch export svg dogrudan_sdr_A.kicad_sch -o /tmp/A_svg >/dev/null
    echo "   /tmp/A.pdf  ve  /tmp/A_svg/"
fi

echo
echo "ac:  kicad $(pwd)/dogrudan_sdr_A.kicad_pro"
