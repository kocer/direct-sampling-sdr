...
def ball_atama(ad):
    """ball_atama.json'dan net -> FPGA ball haritasi. Yoksa None.

    NEDEN VAR: sema ureticileri banka pinlerini ALFABETIK siralayip
    veri yollarina zip'liyordu. "A10, A11, B2..." sirasi ball'larin
    fiziksel diziliminde hicbir sey ifade etmiyor; SDRAM'in DQ0
    bacagina kartin ote ucundaki bir ball dusebiliyor ve yol butun
    demeti capraz kesiyor.

    ECP5'in kullanici I/O'lari banka icinde birbirinin yerine gecebilir
    — yani bu atama BIZIM secimimiz. ball_atama.py yerlestirilmis
    karttan iki ucu da koridor eksenine izdusurup monoton esliyor.

    Iki gecisli: once bu dosya olmadan uret, yerlestir, sonra
    ball_atama.py'yi kostur ve YENIDEN uret. Yerlesim koordinatlari
    kat planindan geldigi icin tek tur yakinsiyor.
    """
    global _ATAMA
    if _ATAMA is None:
        import json
        yol = os.path.join(os.path.dirname(os.path.dirname(
            os.path.abspath(__file__))), "ball_atama.json")
        try:
            _ATAMA = json.load(open(yol))
        except Exception:
            _ATAMA = {}
    return _ATAMA.get(ad)


def yol_esle(pinler, netler, *adlar):
    """(ball, net) ciftleri. Atama varsa onu, yoksa alfabetigi kullan.

    BIRDEN COK DEMET ALIR. Bir banka birimi cogu zaman iki cevre
    biriminin yolunu tasiyor (iki ADC, DAC'in iki portu, iki PHY).
    Tek ad verince ikinci demet atamasiz kaliyor ve alfabetik
    havuzdan rastgele ball yiyordu: ADC1 0 kesisme verirken ADC2
    31 veriyordu. Havuzlar zaten ayrik — ball_atama.py her demeti
    o demetin SU ANKI ball'lari uzerinden hesapliyor.
    """
    # SAAT HARITASI HER ZAMAN. Saatler tek bir demete ait degil —
    # ADC'nin DCO'su, SDRAM saati, iki PHY'nin RXC/TXC'si farkli
    # sayfalarda. PCLK kisiti demetten bagimsiz oldugu icin harita
    # her cagriya ekleniyor; asagidaki suzgecler zaten o sayfaya ait
    # olmayan girdileri atiyor.
    a = dict(ball_atama("SAAT") or {})
    for ad in adlar:
        a.update(ball_atama(ad) or {})
    if not a:
        return list(zip(pinler, netler))
    # SADECE BU BIRIMIN PINLERI. SDRAM adres hatlarinin bir kismi
    # (SD_A9..A12) banka 0'a tasiyor; atama onlari banka 0 ball'ina
    # esliyor. Banka 7 birimini cizerken o ball'i buraya yazarsak
    # ayni ag iki isim aliyor, baska pin bos kaliyor — ERC bunu
    # multiple_net_names + pin_not_connected diye gordu.
    # HEM ball HEM net bu birime ait olmali. Yalnizca ball'i
    # suzunce, netleri bu sayfada olmayan girdiler kalan havuzundan
    # ball yiyor ama kimseye verilmiyordu — kalan tukendi, IndexError.
    havuz, agac = set(pinler), set(netler)
    a = {n: p for n, p in a.items() if p in havuz and n in agac}
    kalan = [p for p in pinler if p not in set(a.values())]
    out, i = [], 0
    for net in netler:
        if net in a:
            out.append((a[net], net))
        else:
            out.append((kalan[i], net))
            i += 1
    return out
